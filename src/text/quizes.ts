...
const quizes = () => async (ctx: Context) => {    
  ...
  // Function to generate Telegraph link content
  const generateTelegraphContent = (chapters: string[]) => {
    const formattedChapters = chapters.map(ch => 
      `<p><strong>${ch}</strong><br>Usage: <code>/chapter ${ch} 2</code> (for 2 questions)</p>`
    ).join('');
    
    return `
      <h4>Available Chapters:</h4>
      ${formattedChapters}
      <p>Simply copy any chapter name and use with /chapter command</p>
    `;
  };
  ...
  if (chapterMatch) {    
    ...
    try {    
      ...
      if (!filteredByChapter.length) {    
        ...
        const telegraphUrl = `https://telegra.ph/${encodeURIComponent('Available-Chapters-' + Date.now())}`;
        
        await ctx.replyWithHTML(
          `❌ No questions found for chapter "<b>${chapterName}</b>"\n\n` +
          `📚 <a href="${telegraphUrl}">View all available chapters here</a>\n\n` +
          `Then use: <code>/chapter [name] [count]</code>\n` +
          `Example: <code>/chapter living world 2</code>`
        );
        
        // In a real implementation, you would actually create the Telegraph page here
        // using the Telegraph API with the generateTelegraphContent(chapters) HTML
        return;    
      }    
